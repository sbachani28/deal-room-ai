...
with st.sidebar:
    ...
    st.divider()
    st.markdown("**Deal:**")
    deal_name = st.text_input("Deal Name", placeholder="e.g., Woodbury × Redlands")
...
""", unsafe_allow_html=True)
                        ...
                        st.warning(f"⚠️ ProPublica found this org (EIN: {d.get('ein')}) but has no financial filing data on record. Try searching by the institution's full legal name, or check [ProPublica Nonprofit Explorer](https://projects.propublica.org/nonprofits/) directly.")
...
""")
    ...
    # ── Auto Accreditation Analysis ──────────────────────────────────────────
    st.divider()
    st.subheader("⚖️ Accreditation Change-of-Control - Deal Analysis")
...
⏱️ <b>Timeline:</b> {acc['timeline']}<br>
...
⚠️ <b>Cross-Accreditor Deal Detected</b><br>
...
# ── TAB 2: Document Analysis ─────────────────────────────────────────────────
    ...
    # ── Auto-generate from public data ───────────────────────────────────────
    ...
    if st.session_state.analyses:
        ...
        for i, a in enumerate(st.session_state.analyses):
            with st.expander(f"📄 {a['name']} - {a['type']}", expanded=(i == len(st.session_state.analyses) - 1)):
                ...
                try:
                    ...
                    if red_flags:
                        st.markdown('<div class="risk-red"><b>🚨 Red Flags</b><br>' + "<br>".join(f"• {r}" for r in red_flags) + "</div>", unsafe_allow_html=True)
                    if key_findings:
                        st.markdown('<div class="risk-green"><b>✅ Key Findings</b><br>' + "<br>".join(f"• {r}" for r in key_findings) + "</div>", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        st.info("💡 Fetch public data for your institutions in the Public Data tab to auto-fill these fields.")
    ...
    with col2:
        if st.button("Calculate CFI", type="primary"):
            ...
            if "error" in result:
                ...
                comp_data = {
                    "Primary Reserve": (comps["primary_reserve_ratio"], comps["primary_reserve_strength"], "≥ 0.20 target"),
                    "Equity Ratio": (comps["equity_ratio"], comps["equity_ratio_strength"], "≥ 0.25 target"),
                    "Net Income": (comps["net_income_ratio"], comps["net_income_strength"], "≥ 0.02 target"),
                    "Return on Net Assets": (comps["rona"], comps["rona_strength"], "≥ 0.03 target"),
                }
                for name, (ratio, strength, target) in comp_data.items():
                    ...
                    st.markdown(f'<div class="{cls}"><b>{name}</b>: {ratio:.4f} → Strength: {strength:.2f} ({target})</div>', unsafe_allow_html=True)
